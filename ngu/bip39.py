...
def b2a_words(msg):
    ...
    # add checksum, 4 to 8 bits
    chk = sha256(msg).digest()[0]
    c_bits = l // 4
    val <<= c_bits
    val |= (chk >> 8-c_bits) & ((1<<c_bits)-1)

    num_words = ((l * 8) + c_bits) // 11

    result = []
    for i in range(num_words):
        val, re = divmod(val, 0x800)
        result.append(re)
...
def master_secret(words, pw=b''):
    # BIP-39 seed words + passphrase => master secret for bip32
    salt = b'mnemonic' + pw
    try:
        import ngu
